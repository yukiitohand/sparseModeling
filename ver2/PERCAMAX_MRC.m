function [ percmax_mrc,percmax_cnd,mcc ] = PERCAMAX_MRC( Y,A,gamma,Gamma )
% [ percmax_mrc,percmax_cnd,mcc ] = PERCAMAX_MRC( Y,A,gamma,Gamma )
%
% This function tests the PERC-AMAX based model recovery condition (MRC) of
...
% The PERC-AMAX MRC is composed of the two conditions, minimum coefficient
% condition(MCC) and PERC-AMAX condition.
%
%  1) PERC-AMAX condition: max(A'*PG_ort*y,[],1) < gamma*PERC(Gamma)
...
%           in the range[1,N])
% Outputs
%   percamax_mrc : boolean array [1,M], evaluating PERC-AMAX & MCC
%                  True->the condition met False->not
%   percamax_cnd : boolean array [1,M], evaluating PERC-AMAX condition
%                  True->the condition met False->not.
...
lhs1 = max(A'*Pg_ort*Y,[],1);
perc = PERC(A,Gamma);
percmax_cnd = (lhs1 < gamma*perc);

% inv(Ag'*Ag) * (Ag'*s-gamma) > 0
lhs2 = AgtAg \ (Ag'*Y-gamma);
mcc = all(lhs2>0, 1);

percmax_mrc = and(percmax_cnd,mcc);
